 ...
 */
export async function fetchTrace(rpc: DevtoolsRpcFace, sessionId: string): Promise<TraceSnapshot> {
  const res = await rpc.call('/dsh-devtools', 'trace', { sessionId })
  if (res.ok && res.value !== undefined) return res.value as TraceSnapshot
